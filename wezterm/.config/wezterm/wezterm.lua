...
config.font_size = 12.0 -- Match Neovim
...
config.leader = { key = "Enter", mods = "CTRL", timeout_milliseconds = 2000 }
config.keys = {
  {
    mods = "LEADER",
    key = "c",
...
wezterm.on("update-status", function(window, pane)
  -- 1. Check leader state immediately; this is a cheap call.
  ...
  if leader_active then
    ...
    -- 2. Use the 'pane' object passed into the function instead of window:mux_window()
    -- This is much lighter as 'pane' is already provided by the event.
    local tab = pane:tab()
    local is_first_tab = tab and tab:get_title() == window:mux_window():tabs_with_info()[1].title -- Still expensive

    -- BETTER: Simplified logic. Just use a standard divider unless you
    -- strictly need it to merge with the first tab's color.
    if tab_style == "rounded" then
